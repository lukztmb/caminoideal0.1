...
db = cli["datos"]
coleccion_bibliografias = db["bibliografias"]

def crear_bibliografia(titulo, autor, enlace, descripcion):
    bibliografia = {
        "titulo": titulo,
        "autor": autor,
        "enlace": enlace,
    ...
    resultado = coleccion_bibliografias.insert_one(bibliografia)
    print(f"Bibliografía creada con ID: {resultado.inserted_id}")

def leer_bibliografias():
    print("\nLista de bibliografías:")
    for biblio in coleccion_bibliografias.find():
        print(f"ID: {biblio['_id']} | Título: {biblio['titulo']} | Autor: {biblio['autor']} | Enlace: {biblio['enlace']}")

def actualizar_bibliografia(id_biblio, nuevo_titulo=None, nuevo_autor=None, nuevo_enlace=None, nueva_descripcion=None):
    campos_a_actualizar = {}
    if nuevo_titulo:
    ...
    if campos_a_actualizar:
        resultado = coleccion_bibliografias.update_one(
...
def eliminar_bibliografia(id_biblio):
    resultado = coleccion_bibliografias.delete_one({"_id": ObjectId(id_biblio)})
...
def cargar_bibliografias_desde_diccionario():
    print("\nCargando bibliografías desde el diccionario...")
    diccionario_biblios = obtenerDiccionario()
    for categoria, biblios_lista in diccionario_biblios.items():
        ...
        for biblio_data in biblios_lista:
            existente = coleccion_bibliografias.find_one({"titulo": biblio_data["titulo"], "autor": biblio_data["autor"]})
            if existente:
...
if __name__ == "__main__":
    ...
    cargar_bibliografias_desde_diccionario()
