...
class CursosCRUD:
    def __init__(self):
        """Inicializa la conexión a la base de datos y la colección de cursos."""
        try:
    ...
    def crear_curso(self, id_neo4j, nombre, descripcion, nivel, temas, prerrequisitos, enciclopedia_desbloqueada):
        ...
        try:
            resultado = self.collection.insert_one(curso)
    ...
    def leer_cursos_por_nombres(self, lista_nombres):
        return list(self.collection.find( 
                ...
                {"enciclopedia_desbloqueada": 1} 
        ))

    def actualizar_curso(self, id_curso_mongo, datos_para_actualizar):
    ...
    def eliminar_curso(self, id_curso_mongo):
    
        if not ObjectId.is_valid(id_curso_mongo):
            print(f"Error: El ID de curso '{id_curso_mongo}' no es válido.")
            return False
        try:
            resultado = self.collection.delete_one({"_id": ObjectId(id_curso_mongo)})
    ...
    def cargar_cursos_desde_diccionario(self):

        print("\nCargando cursos desde el diccionario...")
        diccionario_cursos_data = self.obtenerDiccionarioCursos()
        count_agregados = 0
        count_omitidos = 0
...
if __name__ == "__main__":
    ...
    curso_py_ia = self.collection.find_one({"id_neo4j": "PYAI001"})
    if curso_py_ia:
    ...
    if id_nuevo_curso_mongo: # Usamos el ID que retornó crear_curso
        ...
        curso_fintech_a_eliminar = self.collection.find_one({"id_neo4j": "FINANZASIA016"})
